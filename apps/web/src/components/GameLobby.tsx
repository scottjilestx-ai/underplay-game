...
export function GameLobby({ startError, onStart }: Props) {
  ...
  return (
    <div className="min-h-[100dvh] bg-black flex flex-col items-center justify-center p-6 overflow-y-auto">
      ...
      <div className="w-full max-w-lg rounded-2xl bg-zinc-900/90 border border-zinc-800 p-6 shadow-2xl">
        <Link
          href="/"
          ...
          {opponents.slice(0, opponentCount).map((opp, i) => (
            ...
            >
              <input
                ...
                maxLength={20}
                disabled={mode === "cpu" && false}
                className="w-full bg-transparent text-white font-medium text-sm focus:outline-none border-b border-zinc-800 pb-1 focus:border-emerald-500/40"
        ...
        <div className="flex flex-col gap-2 mb-2">
          {STACK_DISPLAY_OPTIONS.map((opt) => (
            <button
              key={opt.id}
              type="button"
              onClick={() => setStackDisplay(opt.id)}
            ...
            >
              <span
                className={`text-sm font-semibold ${
                  stackDisplay === opt.id ? "text-emerald-100" : "text-zinc-300"
                }`}
              >
                {opt.label}
              </span>
        ...
        <p className="mt-5 text-center text-zinc-600 text-xs">
