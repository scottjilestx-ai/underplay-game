...
/** Flip back → face at source; parent must stay put until this finishes. */
function CardFlipAtSource({
  card,
  deckId,
  delay,
}: {
  ...
  return createPortal(
    <div className="pointer-events-none fixed inset-0 z-[100]">
      {specs.map((spec) => {
        ...
        const sourceHoldS = isPlayFlight
          ? flipAtSource
            ? FLY_FLIP_REVEAL_S
            : FACE_UP_SOURCE_HOLD_S
          : 0;
        ...
        return (
          <motion.div
            ...
            style={{ perspective: flipAtSource ? 1000 : undefined }}
            initial={{
              left: spec.from.left,
              top: spec.from.top,
            ...
            animate={{
              ...
              opacity: 1,
              rotate: isPlayFlight ? (spec.delay / FLY_DURATION_S) * 4 : spec.delay * 3,
            }}
            transition={
              isPlayFlight
                ? {
                    delay: spec.delay + sourceHoldS,
          ...
          >
            {flipAtSource ? (
              <CardFlipAtSource card={spec.card} deckId={deckId} delay={spec.delay} />
            ) : (
              <CardFaceContent
                card={spec.card}
